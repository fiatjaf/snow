...
object RelayTest extends TestSuite {
  /**
    * Note: this is a pretty poorly written test and surely
    * can/should be cleaned up. However, when running this test
    * with `debugOn=true` passed to `Relay.mkResourceForIO` it
    * can be seen in the output that the test:
       - creates two subscriptions:
           1. a subscription for kind 0 events (internally this is given subid 0)
           2. a suscription for kind 1 events (internally this is given subid 1)
  ...
  val tests = Tests {
    test("connect to relay and subscribe") {
      ...
      val program =
        Relay.mkResourceForIO(uri"wss://relay.damus.io", debugOn = true).flatMap {
          relay => 
              (
              relay.subscribe(
          ...
          case ((stored, live),(stored2, live2)) =>
